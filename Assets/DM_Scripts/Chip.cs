...
public class Chip : AnimationObject
{
	...
	public void RunDrop()
	{
		if (destroing || isCombineBomb || GameMain.main == null || (!GameMain.main.isPlaying && !GameMain.main.isGameResult) || GameMain.main.doingStartCameraTween)
		{
			return;
		}
	...
	{
		if ((bool)aniController && !isPlayingClickAnimation && MonoSingleton<PopupManager>.Instance.CurrentPopup == null)
		{
	...
	private void CommonCrush(bool enableCollect = true)
	{
		...
		{
			GameMain.main.DecreaseCollectCandy(base.transform.position, id);
		}
	}
	...
	public void CombineBomb(int toX, int toY)
	{
		...
		isCombineBomb = true;
		CommonCrush();
		base.transform.DOMove(slotPosition, GameMain.BlockDropDelayTime);
#if ENABLE_MEC
	...
	private IEnumerator<float> WaitCombineBomb()
	{
#if ENABLE_MEC
        yield return Timing.WaitForSeconds(GameMain.BlockDropDelayTime);
#else
        yield return GameMain.BlockDropDelayTime;
#endif
        ParentRemove();
		HideChip(enableCollect: false);
	}
	...
	{
		if (!destroyable || destroing || !parentSlot || !parentSlot.slot)
		{
			return;
		}
		StopBounceEffect();
