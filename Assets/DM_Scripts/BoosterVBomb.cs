...
public class BoosterVBomb : Booster
{
	...
	public override void UseBooster(bool isTutorial = false)
	{
		if (CheckEnableBooster(isTutorial))
		{
			...
			guide.textGuide.text = "Touch to launch the rocket vertically from the touch point.";
			if ((bool)guide)
			{
	...
	private IEnumerator UseCandyPack()
	{
		...
		while (onSelect)
		{
			...
			if (targetSlot != null)
			{
				...
				Transform transform = SpawnStringBlock.GetSpawnBlockObjectVBomb(chipID).transform;
				ColorVBomb component = transform.GetComponent<ColorVBomb>();
