...
{
	private enum LangConvertFormatType
	{
		Level,
		Point,
		Star,
	...
	public bool EnableCookappsCrossPromotion;

	public bool EnableIOSInAppRating = true;

	public int InterstitalGameClearCount = 5;

	public string faqCsUrl = "dotmobstudio@gmail.com";
	...
	public bool EnableAndroidAppQuitCrossPromotionPopup;

	public int[] TutorialOpenLevel = new int[8];

	public LiteDailyBonusTable dailyBonusTable = new LiteDailyBonusTable();
	...
	{
		return GetLangCode(GlobalSetting.GetSystemLanguage());
	}
    ...
    //Load infor Booster shops from Json
	private void LoadTableSpecItem(PacketDataSpecItem items)
	{
		...
		//id : 29,30,31,32 : LIST VERTICAL ROCKET
		//id : 25,26,27,28 : LIST HORIZONTAL ROCKET
		//id : 8,15,16,17 : LIST BOMB
		//id : 7,11,12, 13 : LIST HUMMER
	...
	{
      //  Debug.Log("Jewel Hunter ServerDataTable.LoadGameLevelDataFromLocalFile");
			...
			if (fileContents.Length > 10 && fileContents.Substring(0, 10).StartsWith("{\"game"))
			{
				try
				{
					...
					if (resPacketLoadGameData != null)
					{
                        ...
                        //Debug.Log("Jewel Hunter ServerDataTable.LoadGameLevelDataFromLocalFile totalLevel1:" + resPacketLoadGameData.game.Length);
                    }
                }
				catch (Exception ex)
				{
                    Debug.Log("Jewel Hunter ServerDataTable.LoadGameLevelDataFromLocalFile ex:" + ex.Message);
                }
            }
			else
			{
				...
				if (array != null)
				{
                    ...
                    //Debug.Log("Jewel Hunter ServerDataTable.LoadGameLevelDataFromLocalFile totalLevel2:" + array.Length);
                }
            }
			return true;
		}
	...
	{
       // Debug.Log("Jewel Hunter ServerDataTable.LoadGameMapDataFromLocalFile");
		...
		if (!string.IsNullOrEmpty(fileContents))
		{
			if (fileContents.Length > 10 && fileContents.Substring(0, 10).StartsWith("{\"map"))
			{
				...
				{
                   // Debug.Log("Jewel Hunter ServerDataTable.LoadGameMapDataFromLocalFile ex:" + ex.Message);
                }
            }
			else
			{
	...
	private void LoadTableSpecOption(PacketDataOption[] lists)
	{
		if (lists != null)
		{
			...
			SetOptionValue("CP_ENABLE", ref EnableCookappsCrossPromotion);
			for (int j = 1; j <= 8; j++)
			{
			...
			dailyBonusTable.SetTableFromOptionTable();
		}
	}
	...
	public int GetBoosterItemIndex(Booster.BoosterType boosterType)
	{
		switch (boosterType)
		{
	...
	public static Booster.BoosterType GetBoosterTypeFromServerItemIndex(ServerItemIndex itemIndex)
	{
		switch (itemIndex)
		{
		case ServerItemIndex.BoosterHammer:
			return Booster.BoosterType.Hammer;
		case ServerItemIndex.BoosterCandyPack:
			return Booster.BoosterType.CandyPack;
		case ServerItemIndex.BoosterHBomb:
			return Booster.BoosterType.HBomb;
		case ServerItemIndex.BoosterVBomb:
			return Booster.BoosterType.VBomb;
		default:
			return Booster.BoosterType.NONE;
		}
	}

	public static bool IsLockedLevel(int gid)
	{
		if (GetLimitLevel() < gid)
		{
			return true;
		}
		return false;
	...
	public bool IsEnableLocalPush(int index)
	{
		if (listEnabledLocalPushIndex == null)
		{
			listEnabledLocalPushIndex = new List<int>();
			if (m_dicTableOption.ContainsKey("LOCAL_PUSH_ENABLE_INDEX"))
			{
				string[] array = m_dicTableOption["LOCAL_PUSH_ENABLE_INDEX"].Split(',');
				for (int i = 0; i < array.Length; i++)
				{
					int result = 0;
					int.TryParse(array[i], out result);
					listEnabledLocalPushIndex.Add(result);
				}
			}
		}
		if (listEnabledLocalPushIndex.Contains(index))
		{
			return true;
		}
		return false;
	...
	public void SetOptionValue(string key, ref bool t)
	{
		if (m_dicTableOption.ContainsKey(key))
		{
			if (m_dicTableOption[key] == "1")
			{
				t = true;
			}
			else
	...
	public void SetOptionValue(string key, ref string t)
	{
		if (m_dicTableOption.ContainsKey(key))
		{
			t = m_dicTableOption[key];
		}
	}
