...
public class Dig : BlockInterface
{
	...
	public override void BlockCrush(int fromCrushId = -1, int subId = -1)
	{
		if (eventCountBorn != GameMain.main.eventCount)
		{
			...
			if (currentAnimatorStateInfo.IsName("Obstacle_D_normal_lv3_Candy") || currentAnimatorStateInfo.IsName("Obstacle_D_normal_lv2_Candy") || currentAnimatorStateInfo.IsName("Obstacle_D_normal_lv3") || currentAnimatorStateInfo.IsName("Obstacle_D_normal_lv2"))
			{
	...
	private IEnumerator BlockCrush_Coroutine()
	{
		...
		case IBlockType.Digging_HP3_Bomb2:
			CPanelGameUI.Instance.ThrowPurchasedItemEffectForDigging(SpawnStringEffectType.SuccessBuyMove, 5, BoardManager.main.GetSlotPosition(sx, sy));
