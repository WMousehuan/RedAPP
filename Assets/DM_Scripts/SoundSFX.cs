...
public static class SoundSFX
{
	...
	static SoundSFX()
	{
		...
		dicString.Add(0, "SR_ingame_sfx_combo_01");
		dicString.Add(1, "SR_ingame_sfx_combo_02");
		dicString.Add(2, "SR_ingame_sfx_combo_03");
		dicString.Add(3, "SR_ingame_sfx_combo_04");
		dicString.Add(4, "SR_ingame_sfx_combo_05");
		dicString.Add(5, "SR_ingame_sfx_combo_06");
		dicString.Add(6, "SR_ingame_sfx_combo_07");
		dicString.Add(7, "SR_ingame_sfx_combo_08");
	...
	public static void CheckMissingAudioFile()
	{
	}

	public static AudioSource Play(SFXIndex index, bool loop = false, float delay = 0f, SoundManager.SongCallBack endCallBack = null)
	{
		...
		bool looping = loop;
        return SoundManager.PlaySFX(clipName, looping, delay, float.MaxValue, float.MaxValue, default(Vector3), endCallBack);
        return null;
	}

	public static AudioSource Play(string clipName, bool loop = false, float delay = 0f, SoundManager.SongCallBack endCallBack = null)
	{
		bool looping = loop;
        return SoundManager.PlaySFX(clipName, looping, delay, float.MaxValue, float.MaxValue, default(Vector3), endCallBack);
        return null;
	...
	public static AudioSource PlayPitch(SFXIndex index, bool loop = false, float delay = 0f, float pitch = float.MaxValue, SoundManager.SongCallBack endCallBack = null)
	{
		if (!dicString.ContainsKey((int)index))
		{
			return null;
		...
		string clipName = dicString[(int)index];
		bool looping = loop;
        return SoundManager.PlaySFX(clipName, looping, delay, float.MaxValue, pitch, default(Vector3), endCallBack);
        return null;
	}
